  ...
  methods: {
    fetchRevenue() {
      const url = this.selectedCategory
        ? `https://forsit-admin-backend.onrender.com/api/analytics/revenue?category=${this.selectedCategory}`
        : 'https://forsit-admin-backend.onrender.com/api/analytics/revenue'
    ...
    renderChart() {
      ...
      const labels = Array.from({ length: dataset.length }, (_, i) => {
        ...
        if (this.selectedPeriod === 'monthly')
          return ['Jan', 'Feb', 'Mar', 'Apr', 'May'][i] || `Month ${i + 1}`
